...
Original file is located at
    https://colab.research.google.com/drive/1-IyUWW0O-Knb3JW-Z_up2X7qehroF-6H
...
def create_comprehensive_results_table():
    ...
    if len(cnn_detection_training_time) > 0:
        metrics_df.loc['CNN Detection Model', 'Training Time (Seconds)'] = cnn_detection_training_time[0]

        if len(cnn_detection_coord_acc_train) > 0:
            metrics_df.loc['CNN Detection Model', 'Accuracy/Coord_Acc Train'] = cnn_detection_coord_acc_train[0]
            metrics_df.loc['CNN Detection Model', 'Accuracy/Coord_Acc Validation'] = cnn_detection_coord_acc_val[0] if len(cnn_detection_coord_acc_val) > 0 else None
            metrics_df.loc['CNN Detection Model', 'Accuracy/Coord_Acc Test'] = cnn_detection_coord_acc_test[0] if len(cnn_detection_coord_acc_test) > 0 else None

            metrics_df.loc['CNN Detection Model', 'MSE Train'] = cnn_detection_mse_train[0]
            metrics_df.loc['CNN Detection Model', 'MSE Validation'] = cnn_detection_mse_val[0] if len(cnn_detection_mse_val) > 0 else None
            metrics_df.loc['CNN Detection Model', 'MSE Test'] = cnn_detection_mse_test[0] if len(cnn_detection_mse_test) > 0 else None

            metrics_df.loc['CNN Detection Model', 'MAE Train'] = cnn_detection_mae_train[0]
            metrics_df.loc['CNN Detection Model', 'MAE Validation'] = cnn_detection_mae_val[0] if len(cnn_detection_mae_val) > 0 else None
            metrics_df.loc['CNN Detection Model', 'MAE Test'] = cnn_detection_mae_test[0] if len(cnn_detection_mae_test) > 0 else None

            metrics_df.loc['CNN Detection Model', 'Precision/IoU Train'] = cnn_detection_iou_train[0]
            metrics_df.loc['CNN Detection Model', 'Precision/IoU Validation'] = cnn_detection_iou_val[0] if len(cnn_detection_iou_val) > 0 else None
            metrics_df.loc['CNN Detection Model', 'Precision/IoU Test'] = cnn_detection_iou_test[0] if len(cnn_detection_iou_test) > 0 else None
            ...
            metrics_df.loc['CNN Detection Model', 'R2 Score Train'] = cnn_detection_r2_train[0]
            metrics_df.loc['CNN Detection Model', 'R2 Score Validation'] = cnn_detection_r2_val[0] if len(cnn_detection_r2_val) > 0 else None
            metrics_df.loc['CNN Detection Model', 'R2 Score Test'] = cnn_detection_r2_test[0] if len(cnn_detection_r2_test) > 0 else None
...
def create_fast_rcnn_model(input_shape=(224, 224, 3), num_classes=4, max_boxes=10):
    '''Create a Fast R-CNN detection model architecture'''
    inputs = Input(shape=input_shape)

    # Use transfer learning with ResNet50 backbone
    ...
    outputs = tf.keras.layers.Lambda(
        lambda x: tf.concat([
            tf.nn.softmax(x[..., :num_classes], axis=-1),  # Class probabilities
            tf.nn.sigmoid(x[..., num_classes:num_classes+4]),  # Box coordinates (0-1)
            tf.nn.sigmoid(x[..., num_classes+4:])  # Confidence score
        ], axis=-1)
    )(outputs)

    model = Model(inputs=inputs, outputs=outputs)
    print("Fast R-CNN Model Summary:")
    model.summary()
...
def convert_labels_for_fast_rcnn_model(Y, num_classes=4):
    '''Convert labels to match model output format'''
...
def fast_rcnn_detection_loss(y_true, y_pred):
    '''Detection loss with class, box, and confidence components'''
    num_classes = 4
    ...
    valid_mask = tf.expand_dims(true_conf, axis=-1)

    # Classification loss (categorical crossentropy)
    class_loss = -tf.reduce_sum(true_classes * tf.math.log(pred_classes + 1e-7), axis=-1)
    class_loss = class_loss * true_conf

    # Box regression loss (MSE)
    box_loss = tf.reduce_sum(tf.square(true_boxes - pred_boxes), axis=-1)
    box_loss = box_loss * true_conf

    # Confidence loss (binary crossentropy)
    conf_loss = -true_conf * tf.math.log(pred_conf + 1e-7) - (1 - true_conf) * tf.math.log(1 - pred_conf + 1e-7)

    # Combine losses
    total_loss = class_loss + box_loss + conf_loss

    # Average over valid boxes
    num_valid = tf.maximum(tf.reduce_sum(true_conf, axis=-1), 1.0)
    loss_per_sample = tf.reduce_sum(total_loss, axis=-1) / num_valid

    return tf.reduce_mean(loss_per_sample)

def focal_detection_loss(y_true, y_pred, gamma=2.0, alpha=0.25):
    ...
    ce_loss = -true_classes * tf.math.log(pred_classes + 1e-7)
    p_t = tf.where(true_classes == 1, pred_classes, 1 - pred_classes)
    focal_weight = tf.pow(1 - p_t, gamma)
    class_loss = alpha * focal_weight * ce_loss
    class_loss = tf.reduce_sum(class_loss, axis=-1) * true_conf

    # IoU loss for better box regression
    def box_iou_loss(true_box, pred_box):
        # Calculate IoU and return 1 - IoU as loss
        return 1 - calculate_iou_tensor(true_box, pred_box)

    # Smooth L1 loss for box regression
    box_diff = tf.abs(true_boxes - pred_boxes)
    smooth_l1 = tf.where(box_diff < 1, 0.5 * box_diff ** 2, box_diff - 0.5)
    box_loss = tf.reduce_sum(smooth_l1, axis=-1) * true_conf

    # Binary focal loss for confidence
    bce_loss = -true_conf * tf.math.log(pred_conf + 1e-7) - (1 - true_conf) * tf.math.log(1 - pred_conf + 1e-7)
    p_t_conf = tf.where(true_conf == 1, pred_conf, 1 - pred_conf)
    focal_weight_conf = tf.pow(1 - p_t_conf, gamma)
    conf_loss = alpha * focal_weight_conf * bce_loss

    # Weighted combination
    total_loss = class_loss + 2.0 * box_loss + 0.5 * conf_loss
...
def train_fast_rcnn_detection_model(X_train, Y_train, X_val, Y_val):
    ...
    # Convert labels to improved format
    ...
    callbacks = [
        ...
        ModelCheckpoint('best_rcnn_detection_model.h5', save_best_only=True, monitor='val_loss')
    ]
    ...
    return model, history, Y_train_converted, Y_val_converted
...
def calculate_rcnn_detection_metrics(Y_true, Y_pred, num_classes=4):
    ...
    batch_size, max_boxes, _ = Y_pred.shape
    Y_pred_standard = np.zeros((batch_size, max_boxes, 5), dtype=np.float32)

    for i in range(batch_size):
        for j in range(max_boxes):
            class_probs = Y_pred[i, j, :num_classes]
            pred_class = np.argmax(class_probs)
            confidence = Y_pred[i, j, num_classes+4]

            if confidence > 0.5:
                Y_pred_standard[i, j, 0] = pred_class
                Y_pred_standard[i, j, 1:5] = Y_pred[i, j, num_classes:num_classes+4]

    # Convert ground truth back to standard format
    Y_true_standard = np.zeros((batch_size, max_boxes, 5), dtype=np.float32)
    for i in range(batch_size):
        for j in range(max_boxes):
            if Y_true[i, j, num_classes+4] > 0:
                true_class = np.argmax(Y_true[i, j, :num_classes])
                Y_true_standard[i, j, 0] = true_class
                Y_true_standard[i, j, 1:5] = Y_true[i, j, num_classes:num_classes+4]

    # Calculate standard metrics
    Y_true_flat = Y_true_standard.reshape(-1, 5)
    Y_pred_flat = Y_pred_standard.reshape(-1, 5)

    valid_true_mask = np.sum(np.abs(Y_true_flat), axis=1) > 0

    if np.any(valid_true_mask):
        coords_true = Y_true_flat[valid_true_mask, 1:5]
        coords_pred = Y_pred_flat[valid_true_mask, 1:5]

        mse = mean_squared_error(coords_true.flatten(), coords_pred.flatten())
        mae = mean_absolute_error(coords_true.flatten(), coords_pred.flatten())

        if len(coords_true) > 1:
            r2 = r2_score(coords_true.flatten(), coords_pred.flatten())
        else:
            r2 = 0.0
    else:
        mse = mae = r2 = 0.0

    coord_acc = calculate_rcnn_coordinate_accuracy(Y_true_standard, Y_pred_standard)
    avg_iou = calculate_rcnn_average_iou(Y_true_standard, Y_pred_standard)
...
def record_rcnn_detection_performance_metrics(dataset_type, model, X_data, Y_true):
    '''Record performance metrics for Fast R-CNN'''
...
def calculate_rcnn_box_iou(box1, box2):
    '''Calculate IoU between two boxes'''
    _, x1_center, y1_center, w1, h1 = box1
    _, x2_center, y2_center, w2, h2 = box2
...
"""**Calculate average IoU for bounding boxes for Fast R-CNN Model**"""
    ...
    for i in range(len(Y_true)):
        ...
        if len(valid_true_boxes) == 0 and len(valid_pred_boxes) == 0:
            ...
            for true_box in valid_true_boxes:
                best_iou = 0.0
                for pred_box in valid_pred_boxes:
                    iou = calculate_rcnn_box_iou(true_box, pred_box)
                    best_iou = max(best_iou, iou)
                sample_ious.append(best_iou)
...
def visualize_fast_rcnn_predictions(X_test, Y_true, Y_pred, num_samples=20, title="Fast R-CNN Detection Model Predictions", save_name="fast_rcnn_detection_predictions"):
    '''Visualize detection predictions for Fast R-CNN model'''
    ...
    for i, idx in enumerate(indices):
        ...
        for j in range(MAX_BOXES):
            if np.sum(np.abs(Y_true[idx, j])) > 0:
                if Y_true.shape[-1] == 5:  # Standard format
                    class_id, x, y, w, h = Y_true[idx, j]
                else:  # Improved format
                    if Y_true[idx, j, NUM_CLASSES+4] > 0:
        ...
        for j in range(MAX_BOXES):
            if Y_pred.shape[-1] == 5:  # Standard format
                if np.sum(np.abs(Y_pred[idx, j])) > 0.1:
                    class_id, x, y, w, h = Y_pred[idx, j]
                    conf = 1.0
                else:
                    continue
            else:  # Improved format
                ...
                if conf > 0.5:
                    class_id = np.argmax(Y_pred[idx, j, :NUM_CLASSES])
            ...
            if 0 <= pred_class < len(class_names):
                ...
                if Y_pred.shape[-1] > 5:
                    label += f' ({conf:.2f})'
                axes[i].text(x1, y1+height+5, label,
                           color='red', fontsize=8, fontweight='bold',
                           bbox=dict(facecolor='white', alpha=0.8, pad=1))
...
try:
    ...
    rcnn_detection_model, rcnn_detection_history, Y_train_converted, Y_val_converted = train_fast_rcnn_detection_model(
        X_train, Y_train, X_val, Y_val
    )
    ...
    # Convert test labels
    Y_test_converted = convert_labels_for_fast_rcnn_model(Y_test)

    # Evaluate on all datasets
    print("\n--- Training Set Evaluation ---")
    aug_train_coord_acc = record_rcnn_detection_performance_metrics('train', rcnn_detection_model, X_train, Y_train_converted)

    print("\n--- Validation Set Evaluation ---")
    aug_val_coord_acc = record_rcnn_detection_performance_metrics('valid', rcnn_detection_model, X_val, Y_val_converted)

    print("\n--- Test Set Evaluation ---")
    aug_test_coord_acc = record_rcnn_detection_performance_metrics('test', rcnn_detection_model, X_test, Y_test_converted)

    print(f"\nFinal Fast R-CNN Test Coordinate Accuracy: {aug_test_coord_acc:.4f}")
...
aug_pred = rcnn_detection_model.predict(X_test[:20])
visualize_fast_rcnn_predictions(X_test[:20], Y_test_converted[:20], aug_pred, 20,
                              "Fast R-CNN Predictions",
                              "fast_rcnn_predictions")

"""**Plotting Fast R-CNN Training History**"""

# Plot training history
plot_fast_rcnn_training_history(rcnn_detection_history,
