...
Original file is located at
    https://colab.research.google.com/drive/1t-vo35pPRKm97oLwg2RB0J-dVvm8XyHA
...
"""**Create target vector from proposal targets**
...
def create_comprehensive_results_table_with_rcnn():
    ...
    try:
        ...
        if len(detection_training_time) > 0:
            metrics_df.loc['Fast R-CNN Model', 'Training Time (Seconds)'] = detection_training_time[0]
        ...
        for i, model_name in enumerate(model_names[:4]):
            if i < len(acc_train_models):
                ...
                metrics_df.loc[model_name, 'MSE Train'] = mse_train_models[i]
                metrics_df.loc[model_name, 'MSE Validation'] = mse_val_models[i] if i < len(mse_val_models) else None
                metrics_df.loc[model_name, 'MSE Test'] = mse_test_models[i] if i < len(mse_test_models) else None

                metrics_df.loc[model_name, 'MAE Train'] = mae_train_models[i]
                metrics_df.loc[model_name, 'MAE Validation'] = mae_val_models[i] if i < len(mae_val_models) else None
                metrics_df.loc[model_name, 'MAE Test'] = mae_test_models[i] if i < len(mae_test_models) else None

                metrics_df.loc[model_name, 'Precision/IoU Train'] = precision_train_models[i]
                metrics_df.loc[model_name, 'Precision/IoU Validation'] = precision_val_models[i] if i < len(precision_val_models) else None
                metrics_df.loc[model_name, 'Precision/IoU Test'] = precision_test_models[i] if i < len(precision_test_models) else None

                metrics_df.loc[model_name, 'Recall Train'] = recall_train_models[i]
                metrics_df.loc[model_name, 'Recall Validation'] = recall_val_models[i] if i < len(recall_val_models) else None
                metrics_df.loc[model_name, 'Recall Test'] = recall_test_models[i] if i < len(recall_test_models) else None

                metrics_df.loc[model_name, 'F1-Score Train'] = f1_train_models[i]
                metrics_df.loc[model_name, 'F1-Score Validation'] = f1_val_models[i] if i < len(f1_val_models) else None
                metrics_df.loc[model_name, 'F1-Score Test'] = f1_test_models[i] if i < len(f1_test_models) else None

                metrics_df.loc[model_name, 'R2 Score Train'] = r2_train_models[i]
                metrics_df.loc[model_name, 'R2 Score Validation'] = r2_val_models[i] if i < len(r2_val_models) else None
                metrics_df.loc[model_name, 'R2 Score Test'] = r2_test_models[i] if i < len(r2_test_models) else None
        ...
        if len(detection_coord_acc_train) > 0:
            metrics_df.loc['Fast R-CNN Model', 'Accuracy/Coord_Acc Train'] = detection_coord_acc_train[0]
            metrics_df.loc['Fast R-CNN Model', 'Accuracy/Coord_Acc Validation'] = detection_coord_acc_val[0] if len(detection_coord_acc_val) > 0 else None
            metrics_df.loc['Fast R-CNN Model', 'Accuracy/Coord_Acc Test'] = detection_coord_acc_test[0] if len(detection_coord_acc_test) > 0 else None

            metrics_df.loc['Fast R-CNN Model', 'MSE Train'] = detection_mse_train[0]
            metrics_df.loc['Fast R-CNN Model', 'MSE Validation'] = detection_mse_val[0] if len(detection_mse_val) > 0 else None
            metrics_df.loc['Fast R-CNN Model', 'MSE Test'] = detection_mse_test[0] if len(detection_mse_test) > 0 else None

            metrics_df.loc['Fast R-CNN Model', 'MAE Train'] = detection_mae_train[0]
            metrics_df.loc['Fast R-CNN Model', 'MAE Validation'] = detection_mae_val[0] if len(detection_mae_val) > 0 else None
            metrics_df.loc['Fast R-CNN Model', 'MAE Test'] = detection_mae_test[0] if len(detection_mae_test) > 0 else None

            metrics_df.loc['Fast R-CNN Model', 'Precision/IoU Train'] = detection_iou_train[0]
            metrics_df.loc['Fast R-CNN Model', 'Precision/IoU Validation'] = detection_iou_val[0] if len(detection_iou_val) > 0 else None
            metrics_df.loc['Fast R-CNN Model', 'Precision/IoU Test'] = detection_iou_test[0] if len(detection_iou_test) > 0 else None
            ...
            metrics_df.loc['Fast R-CNN Model', 'R2 Score Train'] = detection_r2_train[0]
            metrics_df.loc['Fast R-CNN Model', 'R2 Score Validation'] = detection_r2_val[0] if len(detection_r2_val) > 0 else None
            metrics_df.loc['Fast R-CNN Model', 'R2 Score Test'] = detection_r2_test[0] if len(detection_r2_test) > 0 else None
...
def plot_updated_model_comparison(results_df):
    """
    Create visualization comparing all models including Fast R-CNN
    """
...
plot_updated_model_comparison(final_results_df)